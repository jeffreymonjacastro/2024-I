...
// Traductor de C++ a Lenguaje máquina

class Token {
public:
  enum Type { LABEL, ID, KEYWORD, NUM, EOL, ERR, END };
  // agregar en KeywordType
  enum KeywordType { NOTHING, PUSH, POP, DUP, ADD, SUB, MUL, DIV, GOTO, JMP_LT };
  Type type;
  KeywordType ktype;
  string text;
  Token(Type);
...
Token* Scanner::nextToken() {
  ...
  if (isdigit(c)) {
    ...
    if (input[current] == ':') {
      ...
      else if (!s.compare("mul")) ktype = Token::MUL;
      else if (!s.compare("div")) ktype = Token::DIV;
      // agregar
      else if (!s.compare("goto")) ktype = Token::GOTO;
      else if (!s.compare("jmplt")) ktype = Token::JMP_LT;
...
public:
  // modificar InstrType
  enum InstrType { IPUSH, IPOP, IDUP, IADD, ISUB, IMUL, IDIV,  IGOTO, IJMP_LT };
  string label;
  InstrType type;
  string gotoLabel;
...
void Parser::evaluateInstruction(Instruction* instr) {
  ...
  if (itype == Instruction::IPOP) {
    ...
    case Instruction::IMUL: v = x*y; break;
