...
ImpValue ImpInterpreter::visit(BinaryExp* e) {
  ...
  if (v1.type == TBOOL || v2.type == TBOOL) {
    cout << "Error de tipos: operandos en operacion binaria tienen que ser enteros o floats" << endl;
    exit(0);
  }
  int iv, iv1, iv2;
  float fv, fv1, fv2;
  ...
  switch(e->op) {
    case PLUS : {
      if (v1.type == TFLOAT && v2.type == TFLOAT) {
        fv = fv1 + fv2;
        type = TFLOAT;
      } else if (v1.type == TINT && v2.type == TFLOAT){
        fv = iv1 + fv2;
        type = TFLOAT;
      } else if (v1.type == TFLOAT && v2.type == TINT) {
        fv = fv1 + iv2;
        type = TFLOAT;
      } else {
        iv = iv1 + iv2;
  ...
  else if (type == TFLOAT) result.float_value = fv;
...
ImpValue ImpInterpreter::visit(FloatExp* e){
  ImpValue v;
  v.set_default_value(TFLOAT);
  v.float_value = e->value;
  return v;
}
