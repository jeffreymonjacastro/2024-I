...
const char* Token::token_names[26] = { "ID", "LABEL", "NUM", "EOL", "ERR", "END", "PUSH", "EQ", "GT", "GE", "LT", "LE", "GOTO", "SKIP", "POP", "DUP", "SWAP", "ADD", "SUB", "MUL", "DIV", "STORE", "LOAD", "PRINT", "JMPZ", "JMPN" };
...
Instruction* Parser::parseInstruction() {
  ...
  if (match(Token::SKIP) || match(Token::POP) || match(Token::DUP) ||
      ...
      match(Token::LE) || match(Token::PRINT)) { 
    tipo = 0;
    ttype = previous->type;
  ...
  } else if (match(Token::JMPZ) || match(Token::JMPN) || match(Token::GOTO) ) {
    tipo = 2;
    ttype = previous->type;
    if (!match(Token::ID)) {
